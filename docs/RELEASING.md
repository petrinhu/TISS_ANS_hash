...
| **NuGet** | C# (`langs/csharp`, `TissHash`) | Var `NUGET_ENABLED=true` + Secret `NUGET_API_KEY`. | Conta nuget.org; **API key** (escopo Push, glob `TissHash` ou `*`). **Publicado** em <https://www.nuget.org/packages/TissHash>. |
